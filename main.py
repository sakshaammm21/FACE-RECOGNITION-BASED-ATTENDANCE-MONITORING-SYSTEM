...
def TrackImages():
    try:
        cam = cv2.VideoCapture(0)
        if not cam.isOpened():
            mess._show(title='Camera Error', message='Failed to open camera. Please check your camera connection.')
            return
    ...
    # Keep track of marked attendance to avoid duplicates
    ...
    while True:
        ...
        for (x, y, w, h) in faces:
            ...
            if conf < 70:  # Confidence threshold
                ...
                # Get the name and ID from DataFrame
                try:
                    aa = df.loc[df['SERIAL NO.'] == serial]['NAME'].values[0]
                    ID = df.loc[df['SERIAL NO.'] == serial]['ID'].values[0]
                    
                    # Create unique key for attendance
                    attendance_key = f"{ID}_{date}"
                    ...
                    if attendance_key not in marked_attendance:
                        ...
                        # Update the display
                        tv.insert('', 0, text=str(ID), values=(aa, date, timeStamp))
                        
                    # Display name on video feed
                    cv2.putText(im, str(aa), (x, y + h), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2)
                except IndexError:
